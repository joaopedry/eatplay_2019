...
if(isset($_POST['cadastraUsuarios']))
{
	$sql_criar = "INSERT INTO `users` (`name`, `login`, `password`, `status`, `date`) VALUE ('".$_POST['name']."', '".$_POST['login']."', '".$_POST['password']."', '".$_POST['status']."', '".$_POST['date']."')";
	actionQuery($_SG['link'], $sql_criar);
	unset($_POST['cadastraUsuarios']);
...
{
	$id = $_GET['id'];
	$sql_deletar = "DELETE FROM `users` WHERE `id` = '".$id."'";
	actionQuery($_SG['link'], $sql_deletar);
	unset($_GET['deletarUser']);
	header('location: cadastrausuarios.php');
}
?>